...
master_session_make(struct media_quality *quality, bool use_ptp)
{
  ...
  encode_args.src_ctx = transcode_decode_setup_raw(quality_to_xcode(quality), quality);
...
packet_encrypt(uint8_t **out, size_t *out_len, struct rtp_packet *pkt, struct airplay_session *session)
{
  ...
  uint8_t *write_ptr;
  int ret;
  enum rtp_type type = session->master_session->rtp_session->type;

  // Alloc so authtag and nonce can be appended
  *out_len = pkt->data_len + sizeof(authtag) + sizeof(nonce) - nonce_offset;
  if (type == RTP_BUFFERRED) *out_len += 2; // ensure space for the prefixed length field
  *out = malloc(*out_len);
  write_ptr = *out;
...
packet_send(struct airplay_session *session, struct rtp_packet *pkt)
{
  ...
  uint8_t *write_ptr;
  write_ptr = &buf[0];
  memcpy(write_ptr, &len, 2);
  write_ptr += 2;
  memcpy(write_ptr + 2, pkt->data, pkt->data_len);
        }
      else
        {
  buf = pkt->data;
...
payload_make_setup_stream(struct evrtsp_request *req, struct airplay_session *session, void *arg)
{
  ...
  root = plist_new_dict();
  wplist_dict_add_string(root, "name", PACKAGE_STRING);
  plist_dict_set_item(root, "streams", streams);
  ret = wplist_to_bin(&data, &len, root);
  plist_free(root);
...
response_handler_setup_stream(struct evrtsp_request *req, struct airplay_session *session)
{
  ...
  if (session->server_fd < 0)
    {
      DPRINTF(E_LOG, L_AIRPLAY, "Could not net_connect to data port of '%s' with %s.\n",
        session->devname, (rtp_type == RTP_REALTIME) ? "UDP" : "TCP"
      );
      goto error;
    }
...
airplay_device_cb(const char *name, const char *type, const char *domain, const char *hostname, int family, const char *address, int port, struct keyval *txt)
{
  ...
  device->supported_formats = MEDIA_FORMAT_ALAC | MEDIA_FORMAT_PCM; // BK to check how this gets used.
  ...
  // Default audio quality supported so far
  ...
  if (devcfg && cfg_getint(devcfg, "sample_rate") != CFGF_NONE)
    device->quality.sample_rate = cfg_getint(devcfg, "sample_rate");
  if (devcfg && cfg_getint(devcfg, "bits_per_sample") != CFGF_NONE)
    device->quality.bits_per_sample = cfg_getint(devcfg, "bits_per_sample");
  if (devcfg && cfg_getint(devcfg, "channels") != CFGF_NONE)
    device->quality.channels = cfg_getint(devcfg, "channels");
