...
rtsp_parse(enum airplay_events *event, uint8_t *in, size_t in_len)
{
  ...
  if (strcmp(type, "updateInfo") == 0)
    {
      // device has sent us (new?) info about itself
      // "value" contains a plist dict??
      asprintf(&value, "unhandled plist extraction");
    }
  else
    {
  ...
  else if (strcmp(value, "unhandled plist extraction") == 0)
    *event = AIRPLAY_EVENT_INFO;
  else
...
handle_event(enum airplay_events event)
{
  ...
  switch (event)
	...
	break;
      case AIRPLAY_EVENT_INFO: // do nothing for the moment
      default:
	return;
    }
}
