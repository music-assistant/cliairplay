...
init_settings(struct settings_ctx *settings, enum transcode_profile profile, struct media_quality *quality)
{
  ...
  switch (profile)
	...
	break;
      ...
      // Airplay devices need big-endian if streaming PCM instead of ALAC
      case XCODE_PCM16BE:
	settings->encode_audio = true;
...
//      av_dict_set(&options, "reconnect_at_eof", "1", 0);
  ...
  if (evbuf_io)
      ...
      if (!ifmt)
	{
      ...
      if (strncmp(ctx->settings.in_format, "s24le", strlen("s24le")) == 0) {
        // default sample rate is 44100, channels is 1 for s24le demuxer. Refer ffmpeg -h demuxer=s24le
  ...
  if (options) {
    DPRINTF(E_WARN, L_XCODE, "Didn't recognize all options given to avformat_open_input\n");
    av_dict_free(&options);
  }
