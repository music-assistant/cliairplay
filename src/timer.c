...
#define TIMER_SAMPLE_RATE 48000
#define TIMER_BPS 24
#define TIMER_CHANNELS 2
#define TIMER_BUFSIZE (STOB(TIMER_SAMPLE_RATE, (TIMER_BPS == 24) ? 32 : TIMER_BPS, TIMER_CHANNELS) / 20)
...
setup(struct input_source *source)
{
  ...
  if (TIMER_BPS == 24) {
    DPRINTF(E_DBG, L_PLAYER, "%s:Setting up 24-in-32 noise\n", __func__);
    for (int i = 0; i < TIMER_BUFSIZE; i++) {
      if ((i % 4) == 0) {
        // Zeroes in the top 8 bits
        ctx->noise[i] = 0x00;
      }
      else {
        // Noise in the 24-bits
        ctx->noise[i] = 0x88;
      }
    }
  }
