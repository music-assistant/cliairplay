...
rtp_packet_next(struct rtp_session *session, size_t payload_len, int samples)
{
  ...
  switch (session->type) {
    case RTP_REALTIME:
      ...
      pkt->header[1] = session->type; // M and payload type
      uint16_t seq_16 = htobe16(session->seqnum);
      memcpy(pkt->header + 2, &seq_16, 2);
      break;
    ...
    default:
      DPRINTF(E_LOG, L_PLAYER, "%s:Bug! RTP session type %d not supported, only %d and %d\n",
        __func__, session->type, RTP_REALTIME, RTP_BUFFERRED
      );
  }
